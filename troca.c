...
int main()
{
    ...
    for (i = 0; i < altura; i++)
    {
        for (j = 0; j < largura; j++)
        {
            ...
            imagem[i][j][0] = imagem[i][j][1];  // Move o valor G para a posição R
            imagem[i][j][1] = imagem[i][j][2];  // Move o valor B para a posição G
            imagem[i][j][2] = temp;  // Move o valor temporário (R) para a posição B
        }
    }
